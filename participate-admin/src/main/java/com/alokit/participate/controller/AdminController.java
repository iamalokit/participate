...
public class AdminController {
    ...
    @ResponseBody
    public ApiResponse<Object> updatePassword(@Validated @RequestBody RequestUpdateAdminPassword requestUpdateAdminPassword) {
        int status = adminService.updatePassword(requestUpdateAdminPassword);
        if (status > 0) {
            return ApiResponse.success(status);
        } else if (status == -1) {
            return ApiResponse.failed("提交参数不合法");
        } else if (status == -2) {
            return ApiResponse.failed("找不到该用户");
        } else if (status == -3) {
